...
def is_first_come_first_served(take_out_orders, dine_in_orders, served_orders):
    ...
    for s in served_orders:
        if t_ptr < len(take_out_orders) and take_out_orders[t_ptr] == s:
            t_ptr += 1
        elif d_ptr < len(dine_in_orders) and dine_in_orders[d_ptr] == s:
            d_ptr += 1
        else:
            return False
    if t_ptr < len(take_out_orders) or d_ptr < len(dine_in_orders):
        return False
    return True
